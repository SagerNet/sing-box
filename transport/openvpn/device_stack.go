...
func (d *stackDevice) UpdateConfiguration(configuration Configuration) error {
	...
	if d.logRouteOptions && hasRouteOptions(configuration.Routes) {
		d.options.Logger.Debug("route gateway and metric options are not representable by the stack device; routes are installed by prefix")
...
func (d *stackDevice) bindAddress(destination M.Socksaddr) (netip.Addr, error) {
	...
	if d.options.Configuration.BlockIPv6 {
		return netip.Addr{}, E.New("IPv6 blocked by pushed block-ipv6")
	}
