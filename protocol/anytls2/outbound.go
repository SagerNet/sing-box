...
func RegisterOutbound(registry *outbound.Registry) {
	outbound.Register[option.AnyTLS2OutboundOptions](registry, C.TypeAnyTLS2, NewOutbound)
}
...
func NewOutbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.AnyTLS2OutboundOptions) (adapter.Outbound, error) {
	...
	outbound := &Outbound{
		Adapter: outbound.NewAdapterWithDialerOptions(C.TypeAnyTLS2, tag, []string{N.NetworkTCP, N.NetworkUDP}, options.DialerOptions),
	...
	if options.DialerOptions.TCPFastOpen {
		return nil, E.New("tcp_fast_open is not supported with anytls2 outbound")
	}
	...
	outbound.tlsDialer = tls.NewDialer(outboundDialer, tlsConfig)
	if options.Transport != nil {
		outbound.transport, err = v2ray.NewClientTransport(ctx, outbound.dialer, outbound.server, common.PtrValueOrDefault(options.Transport), outbound.tlsConfig)
		if err != nil {
	...
	if err != nil {
		common.Close(outbound.transport)
		return nil, err
	}
...
func (h *Outbound) dialOut(ctx context.Context) (net.Conn, error) {
	...
	return h.tlsDialer.DialTLSContext(ctx, h.server)
}
