...
func RegisterInbound(registry *inbound.Registry) {
	inbound.Register[option.JuicityInboundOptions](registry, C.TypeJuicity, NewInbound)
}
...
type Inbound struct {
	...
	server       *juicity.Service[int]
	userNameList []string
}

func NewInbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.JuicityInboundOptions) (adapter.Inbound, error) {
	...
	inbound := &Inbound{
		Adapter: inbound.NewAdapter(C.TypeJuicity, tag),
	...
	service, err := juicity.NewService[int](juicity.ServiceOptions{
		Context:     ctx,
		Logger:      logger,
		TLSConfig:   tlsConfig,
	...
	var userUUIDList [][uuid.Size]byte
